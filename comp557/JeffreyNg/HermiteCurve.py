...
class HermiteCurve(Model):
    ...
    def add_point(self, data_point):
        ...
        if(len(self.point_tangent_list) >= 2):
            ...
            column = [self.point_tangent_list[-2][0], self.point_tangent_list[-1][0], self.point_tangent_list[-2][1], self.point_tangent_list[-1][1]] # p(0), p(1), p'(0), p'(1), but vertical
            row = np.array(column).T # tranpose to get a single row
            coMatrix = np.dot(row, self.B_Hermite) # coefficient matrix = G*B
            self.coeff_matrix.append(coMatrix) # append to matrix list
            #-------------- END SOLUTION -------------
    ...
    def evaluate_curve_segment(self, segNo, t):
        ...
        #TODO ---------   BEGIN SOLUTION --------
        p = np.dot(self.coeff_matrix[segNo], [pow(t, 3), pow(t, 2), t, 1])
        m = np.dot(self.coeff_matrix[segNo], [3*pow(t, 2), 2*t, 1, 0])
        #-------------- END SOLUTION -------------
        return (p, m)
